...
int if_in_hash(int key, struct page **hash_t, struct lfu_cache lfu) {
  ...
  // If it is possible(the size is appropriate) to get data from hash_table
  // returns 1 = True
  if (key < lenght) {
    if (hash_t[key] != NULL)
      return 1;
...
void cleaner(struct lfu cache lfu, struct page **hash_t,
  ...
  frequency = current_freq_node->value struct page *del_page =
      (freq_hash_t[frequency])->first;
